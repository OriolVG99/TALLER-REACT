...
const Live = ({ event, onDelete, onUpdate }) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await onDelete(event.id); //onDelete es ara una funció async
    } catch (err) {
      console.error("Error esborrant event:", err);
    } finally {
  ...
  const handleUpdateFromEdit = async (updatedData) => {
    try {
      await onUpdate(updatedData); //onUpdate es async
      setIsEditing(false);
    } catch (err) {
      ...
      <div className="card shadow-sm mb-3">
        ...
        <div className="card-body d-flex justify-content-between align-items-center p-3">
          ...
          <div className="d-flex gap-2">
            ...
            >
              {loadingDelete ? 'Eliminant...' : 'Esborrar'}
            </button>
          </div>
        </div>

        {/* Fila completa al veure fitxa */}
        {isExpanded && (
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowModal(false)}>Cancel·lar</button>
                <button type="button" className="btn btn-danger" onClick={handleConfirmDelete} disabled={loadingDelete}>
                  {loadingDelete ? 'Eliminant...' : 'Esborra'}
                </button>
              </div>
            </div>
          </div>
